...
*/
	...
	public function set_current_page($current_pageno=0)
	{
		...
		if ($this->current_pageno>$this->totalpage)	header("Location:$this->get_url(1)");//////////$this->current_pageno=1////////////
	}
	...
	/* 获取显示"下一页"的代码 @return string*/
	public function next_page()
	{
			if($this->current_pageno<$this->totalpage){
					return '<a title="下一页" href="'.str_input2data($this->get_url($this->current_pageno+1)).'" target="_self">'.$this->next_page.'</a>';
			}
			else
			{
	...
	/* 获取显示"上一页"的代码 @return string*/
	public function pre_page()
	{
			if($this->current_pageno>1){
					return '<a title="上一页" href="'.str_input2data($this->get_url($this->current_pageno-1)).'" target="_self">'.$this->pre_page.'</a>';
			}
			else
			{
	...
	{
			if($this->current_pageno>1){
				return '<a title="首页" href="'.str_input2data($this->get_url(1)).'">'.$this->first_page."</a>";
			}
			else
			{
	...
	/* 获取显示"尾页"的代码 @return string */
	public function last_page()
	{
			if($this->current_pageno<$this->totalpage){
				return '<a title="尾页" href="'.str_input2data($this->get_url($this->totalpage)).'">'.$this->last_page."</a>";
			}
			else
			{
	...
	/* 获取显示"上一分页条"的代码 @return string*/
	public function pre_bar()
	{
			if($this->current_pageno>ceil($this->pagebarnum/2)){
					$pageno=$this->current_pageno-$this->pagebarnum;
					if($pageno<=0)$pageno=1;
					return $this->set_format('<a title="" href="'.$this->get_url($pageno).'" target="_self">'.$this->pre_bar."</a>");
			}
			return $this->set_format('<a title="" href="'.$this->get_url(1).'" target="_self">'.$this->pre_bar."</a>");
	}
	
	/* 获取显示"下一分页条"的代码 @return string*/
	public function next_bar()
	{
			if($this->current_pageno<$this->totalpage-ceil($this->pagebarnum/2)){
					$pageno=$this->current_pageno+$this->pagebarnum;
					return $this->set_format('<a title="" href="'.$this->get_url($pageno).'" target="_self">'.$this->next_bar."</a>");
			}
			return $this->set_format('<a title="" href="'.$this->get_url($this->totalpage).'" target="_self">'.$this->next_bar."</a>");
	}
	
	/* 获取显示跳转按钮的代码 @return string */
	public function select()
	{
			$return='<select title="跳转页码" name="PB_Page_Select" onchange="window.location.href=\''.$this->linkhead.'\'+this.options[this.selectedIndex].value">';
			$s_page=$this->current_pageno-5;
			if($s_page<1)$s_page=1;
			$e_page=$this->current_pageno+5;
			if($e_page>$this->totalpage)$e_page=$this->totalpage;
	...
	/* 获取mysql 语句中limit需要的值@return string */
	public function limit2()
	{
			//return ("共有<font color=red><b>".$this->total."</b></font>条记录。");
			return ('共<font>'.$this->total.'</font>条,每页'.$this->pagesize.'条,<font>'.$this->current_pageno)."</font>页/<font>".$this->totalpage.'</font>页';
	...
	public function pagebar($mode=1)
	{
			...
			switch ($mode)
			{
					...
					case '2':
							...
							$this->first_page='|<<';
							$this->last_page='>>|';
							return $this->first_page().$this->pre_page().'&nbsp;'.$this->select().'&nbsp;'.$this->next_page().$this->last_page();
							break;
					case '3':
							$this->next_page='下页';
							$this->pre_page='上页';
