...
$BIG5oldCiStr='服務器,刻錄,光盤,鼠標,大叔,爺爺,澳大利亞,商場,學生會,報告,朝鮮,忘恩負義,臺灣,盒飯,即使,翻臉,變化,目標,提高,大乳房,不怕,博客,菜譜,殘疾,殘疾人,幕後黑手,卑微,戰略,程序,自助餐,矛盾,出醜,充值,列表,不好意思,輸入,註銷,點擊,升級,任務欄,循環,內存,分辯率,戰鬥機,網絡'; 
$BIG5newCiStr='伺服器,燒錄,光碟,滑鼠,阿拜,阿公,澳洲,百貨公司,班聯會,報備,北韓,背德,本省,便當,便使,變盤,變數,標的,標升,波霸,不甩,部落格,菜單,殘障,殘障人士,藏鏡人,側微,策略,程式,吃到飽,衝突,出糗,儲值,串列,歹勢,登陸,登出,點選,更新,工作列,回圈,記憶體,解析度,戰機,網路';
...
function gb2big5($text){
	...
	$arr_oldCiStr = explode(",",$BIG5oldCiStr);
	$arr_newCiStr = explode(",",$BIG5newCiStr);
	$arr_Conut=count($arr_oldCiStr);
	for($i=0;$i<=$arr_Conut;$i++)
	{
		$rText=str_replace($arr_oldCiStr[$i],$arr_newCiStr[$i],$rText);
	}
    return $rText;
}
